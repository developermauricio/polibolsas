...
?>
			...
			<?php 	// Do we use shortened Featured imgs
					...
					if(function_exists( 'get_post_format' ) && get_post_format( get_the_ID() ) != 'gallery' && get_post_format( get_the_ID() ) != 'video' && has_post_thumbnail()) { 
						$attachment_image = wp_get_attachment_image_src(get_post_thumbnail_id(get_the_ID()), $img_height);
						$att_img_alt = get_post_meta(get_post_thumbnail_id(get_the_ID()), '_wp_attachment_image_alt', true);
			?>			
						<div class="pic">
			...
			if (( function_exists( 'get_post_format' ) && get_post_format( get_the_ID() ) == 'gallery' )) {
				...
				if($images){ 
					...
					$str = "<div id='img_slider_index_template_".get_the_ID()."' class='img_slider' style='opacity:0;'>";
				...
				?>
					...
					<script type="text/javascript">
						jQuery(document).ready(function($) {
							...
							preloadImages($("#img_slider_index_template_<?php echo get_the_ID();?> img"), function () {							
								$("#img_slider_index_template_<?php echo get_the_ID();?>").owlCarousel({
										items: 				1,
										nav: 				true,
										dots:				false,
										autoHeight: 		true,
							...
							function bocShowPostCarousel() {
								$("#img_slider_index_template_<?php echo get_the_ID();?>").fadeTo(0,1);
								$("#img_slider_index_template_<?php echo get_the_ID();?> .owl-item .boc_owl_lazy").css("opacity","1");
							}
			...
			// IF Post type is Video 
					if (( function_exists( 'get_post_format' ) && get_post_format( get_the_ID() ) == 'video')  ) {				

						if($video_embed_code = get_post_meta(get_the_ID(), 'video_embed_code', true)) {
