...
;(function ( $, window, document, undefined ) {
	...
	"use strict";
		...
		$.extend(Plugin.prototype, {
			...
			bindEvents: function() {
				// for medium gallery layout Timer
				var zindexTimeID = 0;
				...
				$fb.on('click.fluidbox', function(e) {
					...
					if(!fb.instanceData.state || fb.instanceData.state === 0) {
						...
						var _photosetrow = $fbWrap.parent().parent().parent('.photoset-row');
						_photosetrow.css('overflow', 'visible');
					...
					// If state exists, we close it
						...
						// control the overflow
						clearTimeout(zindexTimeID);
						zindexTimeID = setTimeout(function() {
							var _photosetrow = $fbWrap.parent().parent().parent('.photoset-row');
							_photosetrow.css('overflow', 'hidden');
						}, 200);
