...
if(!defined('VC_PARALLAX_BG_VERSION')){
	define('VC_PARALLAX_BG_VERSION', '1.5.7');
}

if ( isset( $_GET['uvc-vc-row-class'] ) ) {
	update_option( 'ultimate_custom_vc_row', esc_attr( $_GET['uvc-vc-row-class'] ) );
}
if ( isset( $_GET['uvc-video-fixer'] ) && $_GET['uvc-video-fixer'] != '' ) {
	update_option( 'ultimate_video_fixer', esc_attr( $_GET['uvc-video-fixer'] ) );
}
if ( isset( $_GET['uvc-theme-support'] ) && $_GET['uvc-theme-support'] != '' ) {
	update_option( 'ultimate_theme_support', esc_attr( $_GET['uvc-theme-support'] ) );
}
		...
		function parallax_shortcode($output, $atts, $content){
			...
			$commom_data_attributes .= ' data-theme-support="'.esc_attr( $ultimate_theme_support ).'" ';
				...
				elseif ($bg_type=='u_iframe') {
					wp_enqueue_script('jquery.ytplayer',plugins_url('assets/js/mb-YTPlayer.js',__FILE__), array('ultimate-custom'));
					$v_opts = explode(",",$video_opts);
					$v_img = apply_filters('ult_get_img_single', $video_poster, 'url');
					if(is_array($v_opts)){
						foreach($v_opts as $opt){
